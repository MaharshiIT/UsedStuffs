...
function OpenChatBox(uid, chatid) {
    var win = window.open("ChatBox.aspx?uid=" + uid + "&cid=" + chatid, "ChatWindow" + chatid, "status=0,toolbar=0, menubar=0, width=450, height=550");
    top = top + 50;
        ...
        if (str != "" ) {
            ...
            for (ind = 0; ind < msgs.length; ind++) {
                ...
                for (iind = 0; iind < chats.length; iind++) {
                    try
                    {
                        if (chats[iind] != null && chats[iind].name == "ChatWindow" + msg[1])
                        blnExisting = true;
                    }
                    catch(e){}
                }  
